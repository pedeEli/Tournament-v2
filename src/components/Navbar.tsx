...
const Navbar = ({onMenu}: NavbarProps) => {
  ...
  return <nav className="h-full flex items-center text-2xl bg-primary">
    <ul className="contents">
      ...
      <li id="navbar-contestants" className="mx-6"><a href="/contestants" className="outline-none">Teams</a></li>
      <li id="navbar-groups" className="mx-6"><a href="/groups" className="outline-none">Gruppen</a></li>
      <li id="navbar-tournament" className="mx-6"><a href={`/tournament/${phase === 'brackets' || phase === 'done' || !groups ? 'brackets' : 'groups'}`} className="outline-none">Turnier</a></li>
      <li id="navbar-menu" className="w-10 h-10 mx-6"><button onClick={onMenu} className="w-full h-full outline-none"><MenuSVG/></button></li>
    </ul>
  </nav>
}
